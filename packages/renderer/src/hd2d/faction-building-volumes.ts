...
function goblinCrookedHut(
  root: THREE.Group,
  s: BuildingVolumeDimensions,
  m: FactionBuildingMaterials,
): void {
  const hut = new THREE.Group();
  hut.name = "crooked-hut-body";
  hut.rotation.z = -0.085;
  hut.position.x = -s.width * 0.08;
  root.add(hut);
  roofedBox(
    hut,
    "crooked",
  ...
  door(hut, s.width * 0.12, s.depth * 0.36 + 0.06, s.wallHeight * 0.65, 0.46, m);
  windowPart(hut, -s.width * 0.22, s.wallHeight * 0.55, s.depth * 0.36 + 0.07, m, 0.65);
  leanRoof(
    root,
    "patched-lean-to",
    s.width * 0.42,
  ...
  root.getObjectByName("patched-lean-to")?.position.set(s.width * 0.34, 0, -s.depth * 0.08);
  for (const x of [-s.width * 0.42, s.width * 0.44])
    spike(root, "crooked-roof-stake", [x, s.wallHeight + 0.24, 0], 0.48, m, [
      0,
      0,
      x > 0 ? -0.3 : 0.2,
    ]);
...
function goblinFungusBurrow(
  ...
  door(root, 0, radius * 0.93, s.wallHeight * 0.58, 0.42, m, "burrow-door");
  for (const [x, z, scale] of [
    [-radius * 1.2, 0.1, 0.34],
    [radius * 1.08, -0.25, 0.28],
    [radius * 0.75, radius * 0.8, 0.2],
  ] as const) {
    cylinder(
      root,
      "small-fungus-stem",
...
function goblinBossDen(
  root: THREE.Group,
  s: BuildingVolumeDimensions,
  m: FactionBuildingMaterials,
): void {
  const floorY = s.wallHeight * 0.43;
  for (const x of [-s.width * 0.38, s.width * 0.38])
    for (const z of [-s.depth * 0.32, s.depth * 0.32])
      beam(root, "boss-den-stilt", [x, 0, z], [x * 0.9, floorY, z], 0.09, m.wood);
  box(root, "boss-den-platform", [s.width * 0.94, 0.16, s.depth * 0.85], [0, floorY, 0], m.deck);
  const den = roofedBox(
    root,
    "boss-den",
    s.width * 0.7,
  ...
  den.position.y = floorY;
  const entrance = new THREE.Group();
  entrance.name = "boss-door-assembly";
  entrance.position.y = floorY;
  root.add(entrance);
  door(entrance, 0, s.depth * 0.32, s.wallHeight * 0.48, 0.54, m, "boss-door");
  for (const x of [-s.width * 0.42, s.width * 0.42])
    banner(root, x, s.depth * 0.38, s.wallHeight + s.roofHeight * 0.9, m);
  beam(
    root,
    "boss-ladder-left",
    [-0.28, 0, s.depth * 0.46],
    [-0.28, floorY, s.depth * 0.46],
    0.055,
    m.wood,
  );
  beam(
...
function goblinScrapKeep(
  root: THREE.Group,
  s: BuildingVolumeDimensions,
  m: FactionBuildingMaterials,
): void {
  const blocks = [
    [-0.34, 0.28, 0, 0.54, 0.55],
    [0.25, 0.36, -0.08, 0.46, 0.72],
    [-0.05, 0.58, -0.18, 0.5, 0.42],
  ] as const;
  for (const [x, y, z, w, h] of blocks) {
    box(
      root,
      "stacked-scrap-cell",
      [s.width * w, s.wallHeight * h, s.depth * 0.52],
      [s.width * x, s.wallHeight * y, s.depth * z],
      y > 0.5 ? m.metal : m.stoneShade,
      [0, 0, x * 0.08],
    );
  }
  cylinder(
    root,
    "scrap-watch-cylinder",
    s.width * 0.17,
    s.width * 0.21,
    s.wallHeight * 0.82,
    [s.width * 0.31, s.wallHeight * 0.8, -s.depth * 0.18],
    m.stone,
    7,
  );
  box(
    root,
    "scrap-roof-deck",
    [s.width * 0.88, 0.16, s.depth * 0.62],
    [0, s.wallHeight * 0.98, -s.depth * 0.05],
    m.deck,
    [0, 0, -0.04],
  );
  fenceRun(
    root,
    [-s.width * 0.44, s.depth * 0.3],
    [s.width * 0.44, s.depth * 0.3],
    s.wallHeight * 0.55,
    7,
    m,
  ...
  door(root, -s.width * 0.16, s.depth * 0.33, s.wallHeight * 0.42, 0.5, m, "keep-gate");
  banner(root, s.width * 0.34, -s.depth * 0.18, s.wallHeight + s.roofHeight, m, 0.5);
}

function goblinStabYard(
  root: THREE.Group,
  s: BuildingVolumeDimensions,
  m: FactionBuildingMaterials,
): void {
  ...
  box(
    root,
    "stab-yard-awning",
    [s.width * 0.42, 0.1, s.depth * 0.74],
    [-s.width * 0.27, s.wallHeight * 0.92, 0],
    m.roof,
    [0, 0, 0.16],
  );
  for (const z of [-s.depth * 0.24, s.depth * 0.22]) target(root, s.width * 0.22, 0.72, z, m);
  for (const x of [-0.25, 0, 0.25])
    weaponSpear(root, "practice-blade", x, s.depth * 0.3, 0.92, m, x * 0.4);
}

function goblinSlingRange(
  root: THREE.Group,
  s: BuildingVolumeDimensions,
  m: FactionBuildingMaterials,
): void {
  box(
    root,
    "sling-firing-deck",
    [s.width * 0.92, 0.16, s.depth * 0.32],
    [0, 0.28, s.depth * 0.25],
    m.deck,
  );
  for (const x of [-s.width * 0.4, 0, s.width * 0.4]) {
    beam(
      root,
      "sling-fork-left",
      [x - 0.12, 0.3, s.depth * 0.2],
      [x, s.wallHeight * 0.85, s.depth * 0.2],
      0.055,
      m.wood,
    );
    beam(
    ...
    torus(
      ...
      0.13,
      0.025,
      [x, s.wallHeight * 0.76, s.depth * 0.2],
      m.accent,
      [0, 0, 0],
    );
  }
  for (const x of [-s.width * 0.3, s.width * 0.3]) target(root, x, 0.62, -s.depth * 0.38, m);
  box(root, "ammo-trough", [s.width * 0.55, 0.25, 0.34], [0, 0.2, 0], m.metal);
  for (const x of [-0.45, -0.15, 0.18, 0.43]) boulder(root, "sling-stone", [x, 0.35, 0], 0.11, m);
}

function goblinFeastShack(
  root: THREE.Group,
  s: BuildingVolumeDimensions,
  m: FactionBuildingMaterials,
): void {
  const kitchen = roofedBox(
    root,
    "feast-kitchen",
    s.width * 0.42,
    s.depth * 0.85,
  ...
  kitchen.position.x = -s.width * 0.28;
  box(
    root,
    "feast-canopy",
    [s.width * 0.55, 0.1, s.depth * 0.82],
    [s.width * 0.22, s.wallHeight * 0.72, 0],
    m.cloth,
    [0, 0, -0.1],
  );
  for (const z of [-s.depth * 0.26, s.depth * 0.24])
    box(root, "feast-table", [s.width * 0.44, 0.15, 0.34], [s.width * 0.2, 0.48, z], m.deck);
  fire(root, -s.width * 0.27, s.depth * 0.25, 0.7, m);
  cylinder(
    root,
    "feast-smokestack",
    0.16,
    0.2,
    s.wallHeight * 0.9,
    [-s.width * 0.36, s.wallHeight, -s.depth * 0.18],
    m.stoneShade,
    7,
  );
}

function goblinShamanHollow(
  root: THREE.Group,
  s: BuildingVolumeDimensions,
  m: FactionBuildingMaterials,
): void {
  const radius = Math.min(s.width, s.depth) * 0.34;
  cylinder(root, "hollow-dais", radius * 0.9, radius, 0.22, [0, 0.11, 0], m.stoneShade, 12);
  cone(
    root,
    "shaman-canopy",
    radius * 1.05,
    s.roofHeight,
    [0, s.wallHeight * 0.58 + s.roofHeight / 2, 0],
    m.roof,
    [0, 0, 0],
    9,
  );
  for (let index = 0; index < 6; index += 1) {
    ...
    const z = Math.cos(angle) * radius * 1.25;
    cylinder(
      root,
      "shaman-mushroom-stem",
      0.05,
      0.08,
      0.42 + (index % 2) * 0.16,
      [x, 0.23, z],
    ...
    sphere(
      ...
      0.2,
      [1.3, 0.35, 1.25],
      [x, 0.48 + (index % 2) * 0.15, z],
      index % 2 ? m.accent : m.cloth,
      8,
    );
  }
  totem(root, 0, 0, s.wallHeight * 0.94, m, "shaman-idol");
  torus(root, "ritual-ring", radius * 0.65, 0.055, [0, 0.27, 0], m.accent);
}

function goblinTinkerShed(
  root: THREE.Group,
  s: BuildingVolumeDimensions,
  m: FactionBuildingMaterials,
): void {
  box(
    root,
    "tinker-workshop",
    [s.width * 0.62, s.wallHeight * 0.76, s.depth * 0.72],
    [-s.width * 0.12, s.wallHeight * 0.38, 0],
    m.wall,
  );
  leanRoof(
    root,
  ...
  root.getObjectByName("sawtooth-roof-a")?.position.setX(-s.width * 0.2);
  leanRoof(
    root,
    "sawtooth-roof-b",
    s.width * 0.38,
  ...
  root.getObjectByName("sawtooth-roof-b")?.position.setX(s.width * 0.23);
  beam(
    root,
    "workshop-crane",
    [s.width * 0.42, 0, -s.depth * 0.18],
    [s.width * 0.42, s.wallHeight + 0.5, -s.depth * 0.18],
    0.075,
    m.wood,
  );
  beam(
    root,
    "crane-arm",
    [s.width * 0.42, s.wallHeight + 0.42, -s.depth * 0.18],
    [s.width * 0.08, s.wallHeight + 0.42, -s.depth * 0.18],
    0.065,
    m.wood,
  );
  torus(
    ...
    0.3,
    0.07,
    [s.width * 0.23, 0.62, s.depth * 0.38],
    m.metal,
    [0, 0, 0],
  );
  torus(
    ...
    0.18,
    0.055,
    [-s.width * 0.02, 0.43, s.depth * 0.39],
    m.accent,
    [0, 0, 0],
  );
  box(root, "tinker-bench", [s.width * 0.5, 0.15, 0.36], [0, 0.4, s.depth * 0.45], m.deck);
  cylinder(
    root,
    "tinker-flywheel-axle",
    0.055,
    0.055,
    0.48,
    [-s.width * 0.34, 0.62, s.depth * 0.34],
    m.metal,
    7,
  ...
  torus(
    ...
    0.24,
    0.045,
    [-s.width * 0.34, 0.62, s.depth * 0.44],
    m.accent,
    [0, 0, 0],
  );
  for (const x of [-0.2, 0, 0.2]) {
    spike(root, "bench-tool", [x, 0.62, s.depth * 0.47], 0.34, m, [0, 0, x * 1.4]);
  }
}

function goblinScavengerStore(
  root: THREE.Group,
  s: BuildingVolumeDimensions,
  m: FactionBuildingMaterials,
): void {
  box(
    root,
    "scavenger-warehouse-foundation",
    [s.width * 0.92, 0.16, s.depth * 0.82],
    [0, 0.08, 0],
    m.stoneShade,
  );
  for (const x of [-s.width * 0.4, 0, s.width * 0.4]) {
    for (const z of [-s.depth * 0.34, s.depth * 0.34]) {
      beam(
        root,
        "warehouse-frame-post",
        [x, 0, z],
        [x + (x === 0 ? 0 : Math.sign(x) * 0.05), s.wallHeight * 0.9, z],
        0.075,
        m.wood,
      );
    }
  }
  for (const y of [s.wallHeight * 0.32, s.wallHeight * 0.62]) {
    box(
      root,
      "warehouse-storage-shelf",
      [s.width * 0.82, 0.11, s.depth * 0.64],
      [0, y, -s.depth * 0.03],
      m.deck,
    );
  }
  ...
  for (const [x, z, scale] of [
    ...
    crate(root, s.width * x, s.depth * z, scale, m);
  }
  barrel(root, s.width * 0.36, s.depth * 0.34, 0.78, m);
  barrel(root, -s.width * 0.38, s.depth * 0.33, 0.68, m);
  box(
    root,
    "warehouse-loading-counter",
    [s.width * 0.48, 0.16, 0.34],
    [0, 0.48, s.depth * 0.43],
    m.deck,
  );
  banner(root, s.width * 0.42, -s.depth * 0.36, s.wallHeight + s.roofHeight, m, 0.34);
}
...
function buildGoblin(
  ...
  switch (archetype) {
    case "housing-a":
      return goblinCrookedHut(root, s, m);
    case "housing-b":
      return goblinFungusBurrow(root, s, m);
    case "command-a":
      return goblinBossDen(root, s, m);
    case "command-b":
      return goblinScrapKeep(root, s, m);
    case "training-a":
      return goblinStabYard(root, s, m);
    case "training-b":
      return goblinSlingRange(root, s, m);
    case "community-a":
      return goblinFeastShack(root, s, m);
    case "community-b":
      return goblinShamanHollow(root, s, m);
    case "daily-life-a":
      return goblinTinkerShed(root, s, m);
    case "daily-life-b":
      return goblinScavengerStore(root, s, m);
  }
}
...
export function buildFactionBuildingVolume(
  ...
  switch (faction) {
    case "goblin":
      buildGoblin(design, size, materials, archetype);
      break;
    case "orc-troll":
