 ...
 * store (`.vendor/alepha/src/core/providers/StateManager.ts`'s constructor is a bare `this.store =
 * store`, no clone) — and `Alepha.create()` itself mutates that same object (`state.env = {...}`).
 * Passing the module-level `BODY_PARSER_OPTIONS_SEED` object directly would make every test's
 ...
 * shallow copy per call keeps only the (never-mutated) nested body-parser-options value shared,
 * which is safe.
 */
