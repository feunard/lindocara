...
**Architecture:** `shared/map-events.ts` owns types and defensive parsing; two D1 tables ride the map save transaction; the editor gets an EV overlay, an event tool, and the wireframe's dialog in stock shadcn. Spec: `docs/archive/specs/2026-07-19-map-events-design.md` (Decisions bind).
