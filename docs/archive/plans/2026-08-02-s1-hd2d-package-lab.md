...
- Produces:
  ...
  - `createMoodMixer<M extends Record<string, MoodConfig>>(moods: M, start: keyof M & string, fadeSeconds: number): MoodMixer<keyof M & string>` avec `interface MoodMixer<K> { readonly value: ResolvedMood; readonly name: K; goTo(name: K): void; update(dt: number): boolean }`. **Le générique porte sur l'OBJET, pas sur la clé** : `<K extends string>(moods: Record<K, MoodConfig>, start: K, …)` infère `K` depuis `start` et refuse ensuite `goTo("night")` sur un appel `createMoodMixer({ day, night }, "day", …)` — vérifié au compilateur.
  - `createSky(ctx: Hd2dContext): Sky` avec `interface Sky { mesh: THREE.Mesh; readonly horizon: THREE.Color; apply(mood: ResolvedMood, sunDirection: THREE.Vector3): void; update(dt: number, camera: THREE.Camera): void; dispose(): void }` — `dispose()` libère la `SphereGeometry` et le `ShaderMaterial`, comme toutes les autres fabriques de ressources GPU du package. Le PoC s'en passe parce qu'une page unique ne reconstruit jamais sa voûte ; l'éditeur, si.
