...
- Spec: `docs/archive/specs/2026-07-29-alepha-migration-design.md` (incl. the 2026-07-30 amendment: this tranche runs BEFORE the React tranche and carries the minimal client cutover).
...
- **Vendor patches are AUTHORIZED (owner ruling, 2026-07-30)** when a framework gap blocks a task: patch `.vendor/alepha` directly, prove it with this repo's tests, then apply the same change in `../alepha`, run `yarn v` there (5-min timeout), commit+push it, and `npx alepha vendor sync` back here (sync = its own commit). Prefer a vendor patch over a contorted workaround; still report each patch in the task report so the controller tracks the upstream round-trip.
