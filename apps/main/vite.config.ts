...
// Vite config for the ALEPHA stack (`alepha dev` / `alepha build` auto-load it from the app
// root; the legacy workerd stack keeps its own explicit `vite.legacy.config.ts`). Alepha owns the
// server/plugin wiring (react plugin, tsconfig paths, its own middleware) — this file only adds
// what the framework cannot know about this app:
...
// - `@` alias: the client source root, same as the legacy config and every package tsconfig.
