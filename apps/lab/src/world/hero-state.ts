...
export interface HeroState {
   ...
   *  (`hero.ts`, dans `update()`, APRÈS la boucle d'événements de `stepHero`), pas par la règle
   *  pure `stepHero`. C'est voulu, pour deux raisons :
   *
   ...
   *     lent.
   *
   *  2. `emitHaleine()` (appelée par `hero.ts` dans la boucle d'événements, sur l'événement
   *     "haleine" que `stepHero` rend — Task 7, que le déclencheur soit un pas ou le timer de
   *     repos) lit `state.facing` AVANT que ce même `update()` ne le réécrive plus bas. Dans le
   *     code d'origine, cette lecture voyait toujours le `facing` de l'image PRÉCÉDENTE. L'écrire
   *     à l'intérieur de `stepHero` basculerait cette lecture sur l'image COURANTE, ce qui serait
   *     un écart de comportement : la bouffée d'haleine pointerait dans la nouvelle direction
   *     avant même que le sprite ait été affiché tourné. */
  facing: number;
  room: Room | null;
